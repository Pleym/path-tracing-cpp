...
void render(int width,int height,const std::vector<Object*>& scene,char* outputPath,const Vec3& origin,const Vec3& lookat,
            const Vec3& v_up, double v_fov,int samples_per_pixel,int max_depth)
{
    ...
    double *img=new double[width*height*3];

    // Parcourt chaque pixel et calcule sa couleur
    // OpenMP parallélise les boucles sur les pixels ; l'échantillonnage reste séquentiel par pixel
    ...
    for (int i=height-1;i>=0;--i)
    {
        for (int j=0;j<width;++j)
        {
            ...
            for (int s=0;s<samples_per_pixel;++s)
            {
                ...
                color+=calculateColor(ray,scene,max_depth);
            }

            color=color/static_cast<double>(samples_per_pixel);// Moyenne des échantillons

            // v=0 est le bas du cadre, mais la ligne 0 du PPM est le haut de l'image
            int idx=3*((height-1-i)*width+j);
            img[idx+0]=sqrt(color.x);// Correction gamma
            img[idx+1]=sqrt(color.y);
            img[idx+2]=sqrt(color.z);
        }
    }
...
void renderMLT(int width,int height,const std::vector<Object*>& scene,char* outputPath,const Vec3& origin,const Vec3& lookat,
...
{
    // Material Definition
    Lambertian* red_wall = new Lambertian(Vec3(0.65, 0.05, 0.05),1);
    Lambertian* green_wall = new Lambertian(Vec3(0.12, 0.45, 0.15),1);
    Lambertian* white_wall = new Lambertian(Vec3(0.73, 0.73, 0.73),0.5);
    Lambertian* white_floor = new Lambertian(Vec3(0.73, 0.73, 0.73),1);

    Lambertian* red_material = new Lambertian(Vec3(0.8,0.3,0.3),1);
    ...
    Emissive* light = new Emissive(Vec3(20.0, 20.0, 20.0));

    std::vector<Object*> scene;

    // light source
    scene.push_back(new Sphere(light,0.2, Vec3(0,1.9, -2)));

    // Wall and floor
    scene.push_back(new AABB(white_wall, Vec3(-2.0, -2.0, -4.01), Vec3(2.0, 2.0, -4))); // Back Wall
    scene.push_back(new AABB(green_wall, Vec3(-2.0, -2.0, -4.0), Vec3(-1.99, 2.0, 2.0)));  // Left Wall
    scene.push_back(new AABB(red_wall, Vec3(1.99, -2.0, -4.0), Vec3(2.0, 2.0, 2.0)));      // Right Wall
    scene.push_back(new AABB(white_floor, Vec3(-2.0, -2.01, -4.0), Vec3(2.0,-2.0, 2.0))); // ground
    scene.push_back(new AABB(white_wall, Vec3(-2.0, 2.0, -4.0), Vec3(2.0, 2.01, 2.0)));    // ceiling
    scene.push_back(new AABB(white_wall, Vec3(-2.0,-2.0,1.99),Vec3(2.0,2.0,2.0))); //front wall
