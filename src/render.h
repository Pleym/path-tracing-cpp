            ...
            const Vec3& v_up, double v_fov,int samples_per_pixel,int max_depth)
{
    ...
    for (int i=height-1;i>=0;--i)
    {
        for (int j=0;j<width;++j)
        {
            ...
            img[3*(i*width+j)+0]=sqrt(color.x);// Correction gamma
            img[3*(i*width+j)+1]=sqrt(color.y);
            img[3*(i*width+j)+2]=sqrt(color.z);
        }
    }
...
std::vector<Object*> createScene() 
{
    ...
    scene.push_back(new AABB(white_floor, Vec3(-100.0, -2.01, -100.0), Vec3(100.0, -2.0, 100.0))); // ground
